...
static bool nerdIsDragging = false;
...
void HudEditor::render() {
	...
	string speedText = "Speed: " + to_string((int)player->getBlocksPerSecond()) + string(".") + to_string((int)(player->getBlocksPerSecond() * 100) - ((int)player->getBlocksPerSecond() * 100));
	string fpsText = "FPS: " + to_string(g_Data.getFPS());
	string regenText = "Regenerating (" + std::to_string((int)player->getAbsorption()) + "/10)";
	string totalCountAids = "64 blocks";
	string example = "Example Notification";
	string strLength = "kukluxklan.club";
	...
	vec4_t nerdPos = vec4_t(interfaceMod->nerdX, interfaceMod->nerdY + 6, lnerd + interfaceMod->nerdX, interfaceMod->nerdY + 20);
	...
	vec4_t scaffoldRect = vec4_t(scaffold->scX, scaffold->scY, 55 + scaffold->scX, scaffold->scY + 15);
	...
	DrawUtils::drawText(vec2_t(clickGUIRect.x + 17, clickGUIRect.y + 4), &string("cumgui"), MC_Color(255, 255, 255), 1.F, 1.F, true);
	DrawUtils::drawCenteredString(vec2_t(windowSize.x / 2, windowSize.y / 2 - 20), &string("Sukumi Cliarnet"), 1.5f, MC_Color(255, 255, 255), true);
	...
	const bool nerdIsFocused = rectPos.contains(&mousePos);
	...
	if (interfaceMod->info) DrawUtils::drawRectangle(nerdPos, MC_Color(0, 0, 0), 0.5F);
	if (interfaceMod->info) DrawUtils::fillRectangleA(nerdPos, MC_Color(255, 255, 255, 100));
	...
	if (nerdIsDragging) {
		if (leftClickDown) {
			vec2_t diff = vec2_t(mousePos).sub(dragStartHudEditor);
			interfaceMod->nerdY = interfaceMod->nerdY + diff.y;
			interfaceMod->nerdX = interfaceMod->nerdX + diff.x;
			dragStartHudEditor = mousePos;
			// anti idiot
			{
				if (interfaceMod->nerdX + 100 > windowSize.x)
					interfaceMod->nerdX = windowSize.x - 100;

				if (interfaceMod->nerdY + 20 > windowSize.y)
					interfaceMod->nerdY = windowSize.y - 20;

				interfaceMod->nerdX = (float)fmax(0, interfaceMod->nerdX);
				interfaceMod->nerdY = (float)fmax(0, interfaceMod->nerdY);
			}
		}
		else {
			nerdIsDragging = false;
		}
	}
	else if (nerdIsFocused && leftClickDown) {
		dragStartHudEditor = mousePos;
		nerdIsDragging = true;
	}
	shouldToggleLeftClick1 = false;
...
void HudEditor::onLoadSettings(void* confVoid) {
	...
	if (conf->contains("HudEditorMenu")) {
		...
		if (obj.is_null())
			return;
		auto Position = "pos";
		const char* categories[13] = { "Speed","FPS","Position","Watermark","TargetHud","BlockCount","Release","ArrayList","Config","ArmorHud","SessionInfo","PlayerList","nerd"};
		//Speed
		...
		if (obj.contains(categories[12])) {
			...
			if (!SpeedVal.is_null() && SpeedVal.contains(Position)) {
				auto posVal = SpeedVal.at(Position);
				if (!posVal.is_null() && posVal.contains("x") && posVal["x"].is_number_float() && posVal.contains("y") && posVal["y"].is_number_float()) {
					interfaceMod->nerdX = { posVal["x"].get<float>() };
					interfaceMod->nerdY = { posVal["y"].get<float>() };
				}
			}
		}
	}
