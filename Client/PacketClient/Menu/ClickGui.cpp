...
void ClickGui::renderZephyrCategory(Category category, MC_Color categoryColor) {
		...
		for (auto& mod : moduleList) {
			...
			if (allowRender) {
				if (!ourWindow->isInAnimation && !isDragging && rectPos.contains(&mousePos)) {
					DrawUtils::fillRectangle(rectPos, MC_Color(255, 255, 255), 0.5);
					...
					DrawUtils::fillRectangleA(rectPos, mod->isEnabled() ? MC_Color(i) : MC_Color(20, 20, 20, clickGUI->opacity));
				}
			}
					...
					if (clickMod->isExtended) {
						...
						for (auto setting : *settings) {
							...
							case ValueType::BOOL_T: {
								...
								DrawUtils::fillRectangleA(rectPos, MC_Color(20, 20, 20, clickGUI->opacity));
								...
								// Text
								{
									...
									DrawUtils::drawText(textPos, &elTexto, isFocused || setting->value->_bool ? MC_Color(255, 255, 255) : MC_Color(255, 255, 255), true, true, true);
									currentYOffset += textHeight + (textPadding * 2);
								}
								break;
							}
							case ValueType::ENUM_T: {  // Click setting
								...
								float settingStart = currentYOffset;
								{
									...
									windowSize->x = fmax(windowSize->x, DrawUtils::getTextWidth(&len, textSize) + 16);
									DrawUtils::fillRectangleA(rectPos, MC_Color(20, 20, 20, clickGUI->opacity));
									DrawUtils::drawText(textPos, &elTexto, MC_Color(255, 255, 255), textSize, 1.f, false); //enum aids
									//SuperAiddz
									GuiUtils::drawCrossLine(vec2_t(
										currentXOffset + windowSize->x + paddingRight - (crossSize / 2) - 1.f,
										currentYOffset + textPadding + (textHeight / 2)),
							...
							case ValueType::FLOAT_T: {
								// Text and background
								{
									...
									DrawUtils::drawText(textPos, &elTexto, MC_Color(255, 255, 255), textSize, 1.f, false);
								...
								// Slider
								{
									...
									// Visuals & Logic
									{
										...
										rect.z = rect.z - 10;
										DrawUtils::fillRectangle(rect, MC_Color(20, 20, 20), 1);
										DrawUtils::fillRectangleA(rectPos, MC_Color(20, 20, 20, clickGUI->opacity));
										DrawUtils::fillRectangle(rect, MC_Color(20, 20, 20), 1);// Background
										...
										{
											rect.z = rect.x + value;
											DrawUtils::fillRectangle(rect, MC_Color(170, 170, 170), (areWeFocused || setting->isDragging) ? 1.f : 1.f);
										}
										...
										// Drag Logic
										{
											if (setting->isDragging) {
												...
												if (clickGUI->sounds) {
													auto player = g_Data.getLocalPlayer();
													PointingStruct* level = g_Data.getLocalPlayer()->pointingStruct;
													level->playSound("random.click", *player->getPos(), 3, 3);
												}
											}
										}
							...
							case ValueType::INT_T: {
								// Text and background
								{
									...
									DrawUtils::drawText(textPos2, &elTexto, MC_Color(255, 255, 255), textSize, 1.f, false);
	...
	// Draw Category Header
	{
		...
		// Draw category header
		{
			...
			DrawUtils::fillRectangle(rectPos, MC_Color(20, 20, 20), clickGUI->opacity);
			auto i = ColorUtil::interfaceColor(1);
			DrawUtils::fillRectangle(vec4_t(rectPos.x, rectPos.w - 1, rectPos.z, rectPos.w), MC_Color(i), 1 - ourWindow->animation);
			// Draw Dash
			GuiUtils::drawCrossLine(vec2_t(
				currentXOffset + windowSize->x + paddingRight - (crossSize / 2) - 1.f,
				categoryHeaderYOffset + textPadding + (textHeight / 2)),
				MC_Color(20, 20, 20), crossWidth, crossSize, !ourWindow->isExtended);
...
void ClickGui::render() {
	...
	case 0: // Packet
		renderZephyrCategory(Category::COMBAT, MC_Color(255, 255, 255));
		renderZephyrCategory(Category::VISUAL, MC_Color(255, 255, 255));
		renderZephyrCategory(Category::MOVEMENT, MC_Color(255, 255, 255));
		renderZephyrCategory(Category::PLAYER, MC_Color(255, 255, 255));
		renderZephyrCategory(Category::EXPLOIT, MC_Color(255, 255, 255));
		renderZephyrCategory(Category::OTHER, MC_Color(255, 255, 255));
		break;
	case 1: // Vape
