...
bool ConfigCommand::execute(std::vector<std::string>* args) {
	if (args->at(1) == "load" || args->at(1) == "l") {
		...
		clientMessageF("[Skiddle] %sSuccessfully saved config %s%s%s!", GREEN, GRAY, configMgr->currentConfig.c_str(), GREEN);
		clientMessageF("[Skiddle] %sSuccessfully saved %s%s%s!", GREEN, GRAY, SettingMgr->currentSettings.c_str(), GREEN);
		return true;
	}
	return false;
}