...
Regen::Regen() : IModule(0, Category::COMBAT, "Regenerates your health") {
	...
	mode.addEntry("Flareon2", 1);
	registerFloatSetting("Range", &range, range, 1, 7);
	registerIntSetting("BreakTicks", &delay, delay, 0, 20);
	registerIntSetting("BreakDelay", &minedelay, minedelay, 0, 30);
	registerBoolSetting("Progress", &healthvisual, healthvisual);
}
...
const char* Regen::getModuleName() {
	if (mode.getSelectedValue() == 0) name = string("Regen ") + string(GRAY) + string("Flareon");
	if (mode.getSelectedValue() == 1) name = string("Regen ") + string(GRAY) + string("Flareon2");
	return name.c_str();
}
...
void Regen::onTick(C_GameMode* gm) {
	...
	float absorption = player->getMutableAttribute(&attribute)->currentValue;
	if (absorption == 10) return;
		...
		case 0: // Hive
		{
			...
			for (const vec3_ti& offset : blocks) {
				...
				if (destroy/* && tick >= aids*/) {
					...
					if ((enabledticks > minedelay && g_Data.canUseMoveKeys() && !moduleMgr->getModule<Killaura>()->atk) && isregen || (enabledticks > minedelay && g_Data.canUseMoveKeys() && !moduleMgr->getModule<Killaura>()->targetListEmpty) && isregen) {
