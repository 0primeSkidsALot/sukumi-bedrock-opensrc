...
ArrayList::ArrayList() : IModule(0, Category::VISUAL, "Displays enabled modules") {
	...
	registerFloatSetting("Offset", &offsets, offsets, 0.f, 50.f);
	registerBoolSetting("Glow", &glow, glow);
	registerFloatSetting("GlowRadius", &radius, radius, 0.f, 50.f);
	registerFloatSetting("GlowTest", &alpha2, alpha2, -0.10f, 0.10f);
	registerIntSetting("GlowTest2", &ray, ray, 0.f, 16.f);
	registerBoolSetting("LowerCase", &lowercase, lowercase);
	shouldHide = true;
}
...
void ArrayList::onPostRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	if (g_Data.canUseMoveKeys() && !clickGUI->hasOpenedGUI) {
		...
		windowSize.x = windowSize.x - offsets;
		windowSize.y = windowSize.y;
		float textSize = 1.f;
		float yOffset = offsets;
...
#pragma endregion
			auto rectPos2 = vec4_t(xOffset - test, yOffset, windowSize.x, yOffset - test2 + textHeight);

			// Drawing
			auto interfaceColor = ColorUtil::interfaceColor(curIndex);
			//auto interfaceColortwo = ColorUtil::interfaceColortwo(curIndex);
			if (glow) {
				DrawUtils::drawGlow(rectPos2, interfaceColor, alpha2, ray, radius);
			}

			if (lowercase) {
				DrawUtils::toLower(textStr);
			}
