...
void Watermark::onPostRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	if (g_Data.canUseMoveKeys() && !clickGUI->hasOpenedGUI) {
		string name = player->getNameTag()->getText(); name = Utils::sanitize(name); name = name.substr(0, name.find('\n')); string clientName = interfaceMod->getClientName(); string fpsText = string(GRAY) + "[" + string(RESET) + to_string(g_Data.getFPS()) + " FPS" + string(GRAY) + "]"; string skeetLen = "Fatalitysense | " + name + " | " + fpsText; string t = "Fatality         | " + name + " | " + fpsText; string sense = "sense"; string newText = clientName + GRAY + "| " + RESET + WHITE + name; string idk = "S "; string idk2 = string(GRAY) + "ukumi"; string len = idk + idk2; // why u lookin over here
		...
		case 1: // New
			...
			break;
		case 2: // Weird
			DrawUtils::drawGradientText(vec2_t(simpleRect.x, simpleRect.y), &idk, 1, 1, true);
			DrawUtils::drawText(vec2_t(simpleRect.x + flaot - 14 + (textPadding * 2), simpleRect.y - 0), &idk2, MC_Color(), 1, 1, true);
			break;
		case 3: // Skeet
