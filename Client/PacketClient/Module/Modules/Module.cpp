	...
	if (this->enabled != enabled) {
		...
		if (notifications->showToggle && shouldShow && !((GameData::isKeyDown('L') && GameData::isKeyDown(VK_CONTROL)) || GameData::shouldTerminate())) {
			if (notifications->isEnabled()) {
				auto state = enabled ? " was enabled" : " was disabled";
				string noftitle = "Toggled module" ;
				auto notification = g_Data.addNotification(noftitle, this->getRawModuleName() + string(string(state))); notification->duration = 4.0f; if (notifications->color) {
