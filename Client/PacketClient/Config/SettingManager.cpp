...
std::wstring SettingManager::GetRoamingFolderPath() {
	std::string SettingsFolder = (getenv("AppData") + (std::string)"\\..\\Local\\Packages\\Microsoft.MinecraftUWP_8wekyb3d8bbwe\\RoamingState\\SkiddleClient\\");
...
void SettingManager::loadSettings(std::string name, bool create) {
	...
	if (SettingsExists || create) {
		...
		if (SettingsExists) {
			...
			currentSettingsObj["from"] = "SkiddleClient";
		}
		...
		if (g_Data.getLocalPlayer() != nullptr) {
			g_Data.getGuiData()->displayClientMessageF("[Skiddle] %sSuccessfully %s %s%s%s!", GREEN, !SettingsExists ? "created" : "loaded", GRAY, name.c_str(), GREEN);
		}
	}
