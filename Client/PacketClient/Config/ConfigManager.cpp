...
std::wstring ConfigManager::GetRoamingFolderPath() {
	std::string ConfigFolder = (getenv("AppData") + (std::string)"\\..\\Local\\Packages\\Microsoft.MinecraftUWP_8wekyb3d8bbwe\\RoamingState\\SkiddleClient\\Configs\\");
...
void ConfigManager::loadConfig(std::string name, bool create) {
	...
	if (configExists || create) {
		...
		if (configExists) {
			...
			currentConfigObj["from"] = "SkiddleClient";
		}
		...
		if (g_Data.getLocalPlayer() != nullptr) {
			g_Data.getGuiData()->displayClientMessageF("[Skiddle] %sSuccessfully %s config %s%s%s!", GREEN, !configExists ? "created" : "loaded", GRAY, name.c_str(), GREEN);
		}
	} else {
		//if (g_Data.getLocalPlayer() != nullptr) 
