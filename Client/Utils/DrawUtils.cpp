...
void DrawUtils::drawSystemImage(std::string filePath, vec2_t& imagePos, vec2_t& imageDimension, vec2_t& idk, vec2_t& idk2, MC_Color flushColor) {
	texturePtr = new C_TexturePtr();
	C_FilePath imagepath = getenv("APPDATA") + std::string("\\..\\Local\\Packages\\Microsoft.MinecraftUWP_8wekyb3d8bbwe\\RoamingState\\SkiddleClient\\Assets\\") + std::string(filePath);
...
void DrawUtils::drawNameTags(C_Entity* ent, float textSize, bool drawHealth, bool useUnicodeFont) {
	...
	if (refdef->OWorldToScreen(origin, ent->eyePos0.add(0, 0.5f, 0), textPos, fov, screenSize)) {
		...
		fillRoundRectangle(rectPos, MC_Color(0, 0, 0, 170), false);
		vec4_t testRect4 = vec4_t((rectPos.x) + 4, (rectPos.y) + 2, (rectPos.z - 7) - 4, (rectPos.w) - 2);
		if (14 > 0) DrawUtils::drawGlow(testRect4, MC_Color(0, 0, 0, 230), 0.75 / 14, 14, 4);
		if (nametagsMod->underline) {
			fillRectangle(subRectPos, MC_Color(255, 255, 255), 1.f);
		}
		drawText(textPos, &text, MC_Color(59, 8, 173, 179), textSize, 1.f, true);
...
void DrawUtils::drawCircleFilled(vec2_t pos, vec2_t radius, MC_Color color, double quality) {
	float x;
	float y;
	...
	for (double i = 0; i <= 360;) {
		...
		tess_vertex(tesselator, x + pos.x, y + pos.y, 0);
		i = i + quality;
		x = radius.x * (float)cos(i);
		y = radius.y * (float)sin(i);
		tess_vertex(tesselator, x + pos.x, y + pos.y, 0);
		tess_vertex(tesselator, pos.x, pos.y, 0);
		i = i + quality;
	}
