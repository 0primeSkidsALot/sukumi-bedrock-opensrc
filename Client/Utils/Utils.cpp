...
void Utils::systemPlay(std::string name) {
	std::string path = getenv("APPDATA") + std::string("\\..\\Local\\Packages\\Microsoft.MinecraftUWP_8wekyb3d8bbwe\\RoamingState\\SkiddleClient\\Assets\\") + std::string(name);
...
void Utils::makeFolderInPacket(std::string file) {
	std::string path = getenv("APPDATA") + std::string("\\..\\Local\\Packages\\Microsoft.MinecraftUWP_8wekyb3d8bbwe\\RoamingState\\SkiddleClient\\") + std::string(file);
	// Creating File
	if (mkdir(path.c_str()) == -1)
		return;
	else
...
void Utils::makeFolderInAssets(std::string file) {
	std::string path = getenv("APPDATA") + std::string("\\..\\Local\\Packages\\Microsoft.MinecraftUWP_8wekyb3d8bbwe\\RoamingState\\SkiddleClient\\Assets\\") + std::string(file);
	// Creating File
	if (mkdir(path.c_str()) == -1)
		return;
	else
