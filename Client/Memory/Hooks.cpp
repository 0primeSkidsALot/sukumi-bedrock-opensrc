		...
		static auto bobViewHookF = [](__int64 _this, glm::mat4& matrix, float lerpT) {
			...
			if (animations->isEnabled()) {
				...
				if (animations->shouldBlock && g_Data.canUseMoveKeys() && g_Data.isInGame() && !clickGUI->isEnabled()) {
					// 1.7
					if (animations->mode.getSelectedValue() == 0) {
						lerpT = 0.f;
...
__int64 Hooks::RenderText(__int64 a1, C_MinecraftUIRenderContext* renderCtx) {
...

	{
		...
		if (strcmp(screenName.c_str(), "start_screen") == 0) {
			...
			string add = white + "[+] " + string(RESET);
			string remove = white + "[-] " + string(RESET);
			string fix = white + "[*] " + string(RESET);

			float offset = DrawUtils::getFontHeight(1.000f);
			string changeLog = (bold + string("Sukumi") + " \n" + RESET +
				bold + string("Changelogs") + ": \n" + RESET +
				add + "cock and balls \n" +
				"Made with <3 by Lilycat, Enzo, Srxfiq & Tozic \n" +
				"FUCK NIGGERS \n" +
				"FUCK NIGGERS \n" +
				"FUCK NIGGERS \n" +
				"FUCK NIGGERS \n" +
			...
			istringstream stream(changeLog);
			string s;
			int index = 0;
			while (std::getline(stream, s, '\n'))
			{
				DrawUtils::drawText(vec2_t(5.f, 5.f + (index > 0 ? offset * (index) : 0)), &s, ColorUtil::rainbowColor(8, 0.9f, 1.F, -(index * 12.f) * 4.f), 0.985, 1, true);
				index++;
			}
		}
		else {
	...
	if (notificationsMod->isEnabled()) {
		...
		if (notificationsMod->mode.getSelectedValue() == 0) {
			...
			for (auto& notification : notifications) {
				...
				static const float textHeight = (1 + 0.7f * 1) * DrawUtils::getFont(Fonts::SMOOTH)->getLineHeight();
				//float nameLength = DrawUtils::getTextWidth(&substring, 1);
				...
				//&& rect.x <= windowSize.x - margin - fullTextLength - 2 - borderPadding * 2 - 40
				std::string textStr = " " + title + "\n" + message;
				if (notification->duration <= 0.5 && notification->duration > -0.5) {
					vec4_t rect2 = vec4_t(
					...
					vec4_t testRect4 = vec4_t((rect2.x) + 4, (rect2.y) + 2, (rect2.z) - 4, (rect2.w) - 2);
					if (12 > 0) DrawUtils::drawGlow(testRect4, MC_Color(0, 0, 0, 255), 0.75 / 12, 12, 4);
					DrawUtils::drawText(vec2_t(textPos.x, textPos.y), &textStr, MC_Color(255, 255, 255), 1, 1, true);
					auto n = moduleMgr->getModule<Notifications>();
					DrawUtils::fillRoundRectangle(rect2, MC_Color(0, 0, 0, 150), true);
				}
				...
				if (notification->duration > 0.5) {
					...
					// i removed timer not affctive duration cause it buggy - halalware
					// colors:
					// success: 50, 255, 50
				warning: 255, 200, 50;
				error: 255, 50, 50;
					vec2_t textPos = vec2_t(rect.x , rect.y + 1);

					DrawUtils::drawText(vec2_t(textPos.x + 9, textPos.y + 1), &title, MC_Color(255, 255, 255), 1, 1, true);
					DrawUtils::drawText(vec2_t(textPos.x + 9, textPos.y + 7), &message, MC_Color(255, 255, 255), 1, 1, true);
					vec4_t testRect4 = vec4_t((rect.x) + 4, (rect.y) + 2, (rect.z - 7) - 4, (rect.w) - 2);
					if (12 > 0) DrawUtils::drawGlow(testRect4, MC_Color(0, 0, 0, 255), 0.75 / 12, 12, 4);
					DrawUtils::fillRoundRectangle(vec4_t{ rect.x , rect.y, rect.z - 7, rect.w }, MC_Color(0, 0, 0, 150), false);
					//165 165
					DrawUtils::drawBottomLine(vec4_t{ rect.x + 1, rect.y - 2.0f, rect.x +135 - (notification->duration / 5) * 165, rect.w }, MC_Color(255, 255, 255), 3);
				}
				yPos -= 36;
			}
		}
		...
		if (notificationsMod->mode.getSelectedValue() == 1) {
			...
			for (auto& notification : notifications) {
				...
				if (notification->duration <= 0.5 && notification->duration > -0.5) {
					...
					vec2_t textPos = vec2_t(rect2.x + 9, rect2.y + 1);
					vec2_t titlePos = vec2_t(rect2.x + 9, rect2.y + 1);
					auto color = ColorUtil::tozicGayRainbowColor(5, 1.F, 1.F, 1.F);
					vec4_t testRect4 = vec4_t((rect2.x) + 4, (rect2.y) + 2, (rect2.z) - 4, (rect2.w) - 2);
					if (12 > 0) DrawUtils::drawGlow(testRect4, MC_Color(color), 0.75 / 12, 12, 4);
					DrawUtils::drawText(vec2_t(textPos.x, textPos.y), &textStr, MC_Color(255, 255, 255), 1, 1, true);
					auto n = moduleMgr->getModule<Notifications>();
					DrawUtils::fillRoundRectangle(rect2, MC_Color(color), true);
				}
				...
				//error: 255, 50, 50;
					...
					DrawUtils::drawText(vec2_t(textPos.x + 9, textPos.y + 7), &message, MC_Color(255, 255, 255), 1, 1, true);
					vec4_t testRect4 = vec4_t((rect.x) + 4, (rect.y) + 2, (rect.z - 7) - 4, (rect.w) - 2);
					vec4_t solaris = vec4_t((rect.x) + 4, (rect.y) + 2, (rect.x + 135 - (notification->duration / 5) * 165) - 4, (rect.w) - 2);
					if (12 > 0) DrawUtils::drawGlow(testRect4, MC_Color(19, 19, 19, 255), 0.75 / 12, 12, 4);
					if (12 > 0) DrawUtils::drawGlow(solaris, MC_Color(color), 0.75 / 12, 12, 4);
					DrawUtils::fillRoundRectangle(vec4_t{ rect.x , rect.y, rect.z - 7, rect.w }, MC_Color(19, 19, 19, 255), false);
					DrawUtils::fillRoundRectangle(vec4_t{ rect.x, rect.y, rect.x + 135 - (notification->duration / 5) * 165, rect.w }, MC_Color(color), false);
					//165 165
					//DrawUtils::drawBottomLine(vec4_t{ rect.x + 1, rect.y - 2.0f, rect.x + 135 - (notification->duration / 5) * 165, rect.w }, MC_Color(255, 255, 255), 3);
				}
				yPos -= 36;
		...
		if (notificationsMod->mode.getSelectedValue() == 2) {
			...
			float yPos = windowSize.y - 15;
			for (auto& notification : notifications) {
				//if (notification->duration > notification->originalDuration)
					//notification->originalDuration = notification->duration;
				notification->fade();
				if (notification->fadeTarget == 1 && notification->duration <= 0 && notification->duration > -1)
					notification->fadeTarget = 0;
				else if (notification->duration > 0)
				...
				if (notification->duration <= 1) {
					vec4_t rect2 = vec4_t(
						windowSize.x - 80 - notification->duration * 50,
						yPos - margin - textHeight - 4,
						windowSize.x - margin + borderPadding - 2,
						yPos - margin);
					...
					vec4_t testRect4 = vec4_t((rect2.x) + 4, (rect2.y) + 2, (rect2.z) - 4, (rect2.w) - 2);
					//if (12 > 0) DrawUtils::drawGlow(testRect4, MC_Color(0, 0, 0, 255), 0.75 / 12, 12, 4);
					if (notificationsMod->animation.getSelectedValue() == 0) { //fade
						DrawUtils::drawText(vec2_t(textPos.x + 9, textPos.y + 7), &message, MC_Color(255, 255, 255), 1, notification->duration, true);
						DrawUtils::fillRectangleA(rect2, MC_Color(0, 0, 0, (notification->duration* notificationsMod->opacity)));
					}
					if (notificationsMod->animation.getSelectedValue() == 1) { //normal
						DrawUtils::drawText(vec2_t(textPos.x + 9, textPos.y + 7), &message, MC_Color(255, 255, 255), 1, 1, true);
						DrawUtils::fillRectangleA(rect2, MC_Color(0, 0, 0, notificationsMod->opacity));
					}
				}
				...
				if (notification->duration > 1) {
					...
					DrawUtils::drawText(vec2_t(textPos.x + 9, textPos.y + 7), &message, MC_Color(255, 255, 255), 1, 1, true);
					vec4_t testRect4 = vec4_t((rect.x) + 4, (rect.y) + 2, (rect.z - 7) - 4, (rect.w) - 2);
					DrawUtils::fillRectangleA(vec4_t{ rect.x , rect.y, rect.z - 7, rect.w }, MC_Color(0, 0, 0, notificationsMod->opacity));
					DrawUtils::fillRectangleA(vec4_t(rect.x, rect.w - 1.1f, rect.z +50 -notification->duration * (nameLength / 1.9), rect.w), MC_Color(interfaceColor));
				}
				yPos -= 36;
			}
		}
...
void Hooks::LoopbackPacketSender_sendToServer(C_LoopbackPacketSender* a, C_Packet* packet) {
	...
	if (nofall->isEnabled() && nofall->mode.getSelectedValue() == 2)
	{
		if (packet->isInstanceOf<PlayerAuthInputPacket>())
		{
			PlayerAuthInputPacket* inputPacket = reinterpret_cast<PlayerAuthInputPacket*>(packet);

			if (nofall->sentinelSwitch)
			{
				inputPacket->velocity.y = 0.f;
			}
			nofall->sentinelSwitch = !nofall->sentinelSwitch;
		}
	}

	if (disabler->isEnabled()) {

		if (disabler->mode.getSelectedValue() == 4)
		{
...
void Hooks::Actor_swing(C_Entity* _this) {
	...
	if (!(noSwingMod->isEnabled() ||
		// Scaffold
		scaffold->isEnabled() && scaffold->swingmode.selected == 0 && !leftClicking && targetListEmpty ||
		// Breaker
